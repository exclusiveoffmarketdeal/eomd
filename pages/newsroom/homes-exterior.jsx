...
const HomesExterior = () => {
  ...
  const images =
    location === 'beta'
      ? data?.betaPresskitPage?.data?.attributes?.homes_exterior_images?.data
      : data?.prodPresskitPage?.data?.attributes?.homes_exterior_images?.data
  return (
    <>
      <Head>
        <title>Homes Exterior</title>
        <meta name='description' content='Homes Exterior | Exclusive Off Market Deals' />
      </Head>
          ...
          <BasicContainer classnames={'xl:px-16 2xl:px-32 3xl:px-32 4xl:px-32 w-full lg:w-2/3'}>
            <div className='flex flex-col justify-center items-center mt-8 mb-4'>
              <div className='flex flex-row justify-between items-center my-4 w-full'>
                <h1 className='text-3xl lg:text-4xl text-vb_blue-800 font-bold mb-8 lg:mb-4'>Homes Exterior</h1>
